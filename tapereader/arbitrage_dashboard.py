...
if __name__ == "__main__":
    try:
        if sys.platform == "win32":
            os.system("color")
            # Configuração adicional para Windows - melhora compatibilidade com Ctrl+C
            asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
        
        asyncio.run(main())
        
    except KeyboardInterrupt:
        # Captura a interrupção (Ctrl+C) e sai de forma limpa
        print("\n\n✅ Robô de arbitragem finalizado pelo usuário.")
        ...
        print(f"\n\n❌ Erro inesperado ao finalizar: {e}")
        print("Retornando ao menu...")
        import time
        time.sleep(1)